...
export default function Home() {
  ...
  useEffect(() => {
    if (dates?.date?.condominio) dates.getCondominio(dates?.date?.uid, 0)
    console.log(dates?.date)
  }, [dates?.date?.uid])
  return (
    <Layout page="Home">
      <div className=' h-auto md:h-screen p-2 w-full flex-col md:flex pl-20 pr-20 pt-5' >
        {!dates?.date?.condominio &&
          <div className='w-full justify-center items-center'>
            <span className=' text-xl text-red-500'>Você não tem condominios Cadastrado, redireciona-se a pagina de condominio e crie um ou mais condominios.</span>
          </div>
        }
        {dates?.date?.condominio &&
          <>
